...
> 不做「一般来说企业级需要……」式的泛泛而谈。
>
> **另见**：[`JD-GAP-ANALYSIS.md`](JD-GAP-ANALYSIS.md) —— 同一份代码，换成
> 「面向某个具体 Agent 岗位的面试视角」重新排了一遍优先级，结论不一样：
> 本文按**企业级架构**排（正确性 → 生产就绪 → 企业级），
> 那份按**岗位要求**排（运行时层 → 工具调用/上下文 → 基础设施）。
> 两份的差异本身是有效的：**按本文做到 P1 会得到一个很稳的预约系统，
> 但被问「你的工具调用怎么实现的」会当场卡住。**
...
### 快照（`208f074`，含 Harness / react 模式 / 人员准入）
...
* `ruff check .` 与 `mypy`（不带参数、按 `pyproject.toml` 的 `files` 扫
  ...
  本地与 CI 用同一口径，避免"本地绿、CI 红"。
* 上一轮这里写的是"mypy 干净"，而实际上当时有 **2 个报错**（`tests/test_react.py` 的
  `unused-ignore` 与 `ReActAgent` 参数类型）。差异的来源是**口径**：
