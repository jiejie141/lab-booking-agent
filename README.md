...
**表单式下单（模型不在也能约 / 状态映射不说谎 / 代下单要管理员）**、
**后台维护 CRUD（显式 null 不动字段 / 停用即失效 / 没有删除路由 / 设备停用前看占用）**、
**设备级审批（申请即占坑 / 驳回释放时段 / 重复处理 409 / 非管理员看不到待办）**、
**通知（先落库再投递 / 没配 SMTP 不假装成功 / 失败留原因且不重发 / 通知崩了不影响下单）**、
**登录锁定（按「来源地址 # 用户名」计数 / 锁定期间正确口令也进不去 / 过期锁丢掉旧记录 / 锁定请求不写审计）**、
**违约判定（无门禁流水就不判 / 迟到在宽限内不算 / 被门口拦下不算到场 / 只有进门算 / 默认只记不罚 / 豁免不抹判定事实）**、
**预约分页（不传 limit 行为完全不变 / 总数走 X-Total-Count / 越界在入口 422 而不是静默夹值）**、
**备份与恢复（含恢复演练）**。
...
- **限流是单进程的**：进程内滑动窗口，重启即清零，**多副本下等于把配额乘以副本数**。
  生产必须换成 Redis 计数器。
  （补一句实测：随仓库发布的 `docker-compose.yml` 里 **api 只有一个副本**，
  所以这条在默认拓扑下不成立；它只在有人横向扩容时才变成真问题。
  重新评估的触发条件与理由写在 `docs/DELIVERY-READINESS.md` 的 P2-10。）
- **没有 refresh token**：访问令牌 2 小时到期就得重新登录。刻意这么选 ——
  与其签一个 7 天的令牌假装很安全，不如把风险窗口压小。
- **违约判定只在「门禁真的在跑」时成立，且处罚默认关闭**：判定依据是
  `access_events` 里该用户在该时段的**入场**记录。实验室当天一条门禁流水都没有时
  判不了，直接跳过（不猜）。另外 `LAB_NOSHOW_BLOCKING_ENABLED` 默认 `false` ——
  **先只记录不拦人**，确认判定准确后再打开。这是惩罚性功能的正确上线顺序：
  做错方向的代价是冤枉一个守规矩的人，而他自己说不清为什么约不上。
- **通知只有邮件一种通道，且没配 SMTP 就一条都发不出去**：此时通知照写进库、
  状态停在 `pending`，任务报告写明"跳过 N 条：SMTP 未配置"。
  **"我们没配邮件"与"配了但发失败"必须是两种看得出来的状态** —— 否则
  "用户说没收到"只能靠猜。也刻意不做失败自动重发：一封迟到的"预约成功"
  比没有更糟（用户照着它去了实验室，而那条预约可能早取消了）。
- **审批只做两级（通过/驳回），没有多级、会签与委托**：够覆盖"贵重设备要老师点头"，
  但覆盖不了"导师批完还要实验室主任批"这类流程。要做后者是另一个量级的工程，
  本项目明确不做。
- **违约只覆盖「约了不来」一种**：违规操作、损坏设备之类仍然没有记录与约束，
  只能靠管理员在备注里人工记。
