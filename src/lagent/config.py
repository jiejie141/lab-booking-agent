...
class Settings(BaseSettings):
    ...
    # ---- 后台维护（P0-4）---------------------------------------------------
    # ---- 通知（P1-6）-------------------------------------------------------
    # 只做邮件一种通道：学校场景里邮箱是唯一人人都有、又不用接第三方平台的通道。
    # **host 为空即不投递**，但通知照样写进库 —— "没配邮件"与"配了但发不出去"
    # 必须是两种看得出来的状态（详见 notify.smtp_problem）。
    smtp_host: str = ""
    ...
    notify_batch_size: int = Field(default=100, ge=1, le=2000)
    ...
    # ---- 违约与黑名单（P1-8）----------------------------------------------
    # 「约了不来」的判定与处罚。**惩罚性自动化最危险的失败模式是误伤** ——
    # 所以这里刻意拆成「判定」与「处罚」两截，默认只判定、不处罚。
    #
    # 判定依据是门禁流水：预约时段内**没有任何该用户的入场记录**才算违约。
    # 这意味着没接门禁的实验室根本判不了（见 domain/violations.py 的
    # 数据充分性检查）—— 判不了就跳过，绝不猜。

    # 迟到多久仍算到场。与门禁的 ENTRY_GRACE_MINUTES 同一个数量级：
    # 门禁放行了、违约判定却说他没来，是最难解释的那种矛盾。
    noshow_grace_minutes: int = Field(default=15, ge=0, le=120)
    # 统计窗口：只看最近这么多天里的违约次数。设成"永久"会让一次两年前的
    # 失误永远压着一个人，那不是管理，是记仇。
    noshow_window_days: int = Field(default=90, ge=1, le=3650)
    # 窗口内累计多少次违约就暂停预约权限。
    noshow_block_threshold: int = Field(default=3, ge=1, le=100)
    # ★ 是否真的拦住预约。**默认 False**：先跑一段时间只记录不处罚，
    #   拿真实数据确认判定准确（门禁有没有漏刷、宽限够不够）之后再打开。
    #   False 时违约照记、管理员照看得到，只是不拦人 ——
    #   "有数据但没生效"和"没数据"在界面上是两回事，别让它们长得一样。
    noshow_blocking_enabled: bool = False
